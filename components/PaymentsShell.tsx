...
export default function PaymentsShell({ children }: { children: React.ReactNode }) {
  ...
  const workspacePageItems = useMemo(() => navItemsToWorkspacePages(paymentsNavItems), []);
  ...
  return (
    <div className={cn("zamschool-workspace-shell", ws.canvas)}>
      {open && <button className={cn("fixed inset-0 z-30 lg:hidden", ws.overlay)} onClick={() => setOpen(false)} aria-label="Close sidebar" />}
        ...
        <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="border-b border-workspace-border px-3 py-4">
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: "Revenue", value: `K${stats.totalRevenue.toLocaleString()}`, tone: "text-emerald-700" },
                { label: "Pending", value: `K${stats.pendingPayments.toLocaleString()}`, tone: "text-amber-700" },
                { label: "Students", value: String(stats.totalStudents), tone: "text-slate-800" },
                { label: "Overdue", value: String(stats.overduePayments), tone: "text-rose-700" },
              ].map((tile) => (
                <div
                  key={tile.label}
                  className="rounded-workspace-lg border border-workspace-border bg-white p-3 text-center shadow-workspace-xs"
                >
                  <p className={cn("text-sm font-semibold ws-tabular", tile.tone)}>{tile.value}</p>
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-3 pl-2">
              ...
              <Link href="/app/profile" className="group relative w-10 h-10 rounded-full overflow-hidden border border-green-200 bg-green-500 text-white grid place-items-center text-sm font-semibold shadow-sm transition-all hover:ring-2 hover:ring-green-100">
                {avatarUrl ? (
                  <ProfileAvatarImage
                    src={avatarUrl}
                    alt={displayName}

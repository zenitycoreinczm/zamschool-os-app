...
export default function StudentShell({
  ...
  useEffect(() => {
    ...
    fetchShell()
      .then((shell) => {
        if (cancelled) return;
        const className = (shell.shell as { className?: string } | undefined)
          ?.className;
        if (className) setClassLabel(className);
      })
      .catch(() => {
        // Non-critical: class label is a sidebar nicety.
      });
      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-slate-200/80 px-4 py-5">
            <Link href="/app/student" className="flex items-center gap-3">

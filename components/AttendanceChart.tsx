...
export default function AttendanceChart() {
    ...
    const loadAttendance = async () => {
      try {
        const response = await fetch(
          "/api/admin/attendance/summary?range=week",
          {
            cache: "no-store",
          },
        );
  ...
  return (
    <div className="bg-white rounded-workspace-xl w-full h-full min-h-[320px] p-5 flex flex-col shadow-sm border border-slate-100">
          ...
          <div className="h-full grid place-items-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-center px-6">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">
                Weekly attendance trends will appear after roll calls are
                submitted.
              </p>
            </div>
          </div>

...
export default function ParentAbsencePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [childrenRes, absenceRes] = await Promise.all([
        ...
        fetch("/api/parent/absence", { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/parent/absence", {
      ...
      body: JSON.stringify({ studentId: formChild, date: formDate, reason: formReason }),
    });
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">View and justify absence records for your children</p>
        </div>
      ...
      {showForm && (
        <Surface variant="elevated" className="p-5">
          <h2 className="mb-4 text-sm font-semibold text-slate-900">Submit Absence Justification</h2>
      ...
      {error ? (
        ...
        <Surface variant="elevated" className="p-8 text-center">
          ...
          <p className="mt-3 text-sm text-slate-500">No absence records found.</p>
        </Surface>
      ) : (

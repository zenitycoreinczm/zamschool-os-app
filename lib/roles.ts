...
export function normalizeRole(role: string | null | undefined): KnownRole | null {
  ...
  if (KNOWN_ROLES.includes(normalized as KnownRole)) return normalized as KnownRole;
  return ROLE_ALIASES[normalized] || null;
}

export function roleToStoredValue(role: string | null | undefined): string | null {
...
export function roleDatabaseValues(role: string | null | undefined): string[] {
  ...
  const values = new Set<string>([stored, normalized, normalized.toLowerCase()]);
...
export function isSchoolAdministratorRole(role: string | null | undefined): boolean {
